"""GIS Database Layer for FMB Cadastral Records.

Uses SQLite with structured geospatial schemas (GeoJSON, WKT, Bounding Boxes, Vertices, and Segments).
"""
...
class GISDatabase:
    ...
    def _init_db(self) -> None:
        with self._get_connection() as conn:
            ...
            # 2. Line Segments Table (for spatial queries)
            cursor.execute("""
    ...
    def save_parcel(
        ...
        """Save a digitized parcel structure and its geometric primitives into GIS database."""
        meta = metadata or {}
    ...
    def list_parcels(self) -> List[Dict[str, Any]]:
        """List all stored cadastral parcels with summary metrics."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
    ...
    def get_parcel(self, parcel_id: int) -> Optional[Dict[str, Any]]:
        """Retrieve full parcel record by ID."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
    ...
    def get_all_geojson(self) -> Dict[str, Any]:
        """Aggregate all parcels into a unified GeoJSON FeatureCollection."""
