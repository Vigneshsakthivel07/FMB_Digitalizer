"""PostgreSQL + PostGIS Spatial Repository for FMB Cadastral GIS Data.

Provides:
- Native PostGIS Geometry representations (ST_GeomFromText, ST_AsGeoJSON, ST_Area, ST_Buffer).
- Configurable connection parameters via environment variables (PGHOST, PGPORT, PGUSER, PGPASSWORD, PGDATABASE).
- Spatial SQL generation and execution for QGIS, ArcGIS, and PostGIS servers.
"""
...
class PostGISRepository:
    ...
    def generate_postgis_sql(
        ...
        """Generate PostGIS-compliant SQL statements with native spatial constructors.

        Uses ST_GeomFromText, ST_Multi, ST_MakeEnvelope for PostGIS spatial ingestion.
        """
        meta = metadata or {}
        ...
        # 1. MultiLineString WKT for all boundary segments
        lines_wkt_parts = [
        ...
        # 2. MultiPoint WKT for all corner vertices
...
{',\\n'.join(vertex_values)};\n"""
    ...
    def export_sql_dump(
        ...
        """Export full PostGIS schema creation and data insertion script."""
        schema_path = Path(__file__).resolve().parent.parent / "schema_postgis.sql"
        schema_sql = ""
